...
def main() -> None:
    ...
    print(f"\n3. Route elimination (data's own work; v1's running-coupling")
    print(f"   reading is KILLED by sign — corrected in place):")
    print(f"   amplitude ∝ 1/ln  →  tilt +2/L (blue)      — killed by sign")
    print(f"   incoherent (√L)   →  n_s = {1-1/L:.4f} (+{(1-1/L-NS_MEAS)/NS_SIG:.1f}σ) — excluded by data")
    print(f"   COHERENT (∝ ln)   →  n_s = {ns:.4f} — the sole survivor; the")
    print(f"   derivation target: one equal in-phase increment per decade of")
    print(f"   substructure between the mode and the anchor.")
    ...
    print("   the referee precision that would test it, the unique surviving")
    print("   coherent-accumulation route, and the amplitude-tilt split.")
    print("=" * 78)
