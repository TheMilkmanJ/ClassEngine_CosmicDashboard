...
The fit-vs-stack spread (1.4σ) is the identity network's most-loaded joint; the α_c MCMC's α_c
...
α_c = 3α (the registered bet, the α_c MCMC-refereed).
...
|---|---|---|---|
| **The CMB / H₀** | z ≈ 1100 | heavier m_e → earlier decoupling → smaller sound horizon → **H₀ = 69.9** (the tension's mechanism) | production-fit; THE ZERO-PARAMETER EVIDENCE RUN is live (2026-07-13: ε, A_s, n_s, z_on all STATED) |
...
| **Σm_ν de-bias** | fits | dyad-conditional neutrino masses stay healthy (61.4 meV vs DESI ≲72) | recorded; near referee |
...
controls both the dark ages AND the candle room. The recorded lab/Oklo/molecular-absorber
nulls survive under B structurally: every such instrument lives in a screened room (the
μ-audit, entry 66 — molecules only form where ε is off).

## 4. The referees and the killers

the α_c MCMC (α_c, the stack's third factor); the zero-parameter evidence run (live 2026-07-13; the sampled referee deferred by author order); DESI
...
D-line referee (P-027). KILLERS on file: a clean dark-ages detection at the standard
rest frequency; the forest clean at the curve's own prediction; the α_c MCMC landing α_c off 3α
...
(the two skies); [Ubachs-μ-class] (the screened-room nulls); internal: the fingerprint
lattice, THREE_EQUATIONS, entries 59/66–68. Full entries: [BIBLIOGRAPHY.md](BIBLIOGRAPHY.md).
...
lock criterion: the phase-slip freeze-out (entry 77's anneal), Kibble–Zurek class.
