...
|---|--------|---------|-----------------|-----------|
| 1 | Cosmology (CMB/BAO/SN background) | mover | **Stage A booked:** old-BAO SH0ES `bbnfix`; DESI-DR2 SH0ES; DESI-DR2 TRGB (H₀ ~68.4–68.9 both under TRGB). Old-BAO Laplace weak (**ΔlnZ ≈ +0.21**). Nested UN+PC **running** all anchors (mid-run log-evidence not bookable). α_c retune **stopped** (GetDist inconclusive); `conv_desi` retune **stopped** (`g` inconclusive). Bounce: path+waist **derived**; O2 sign derived under ARM; mag **permanent non-claim**; FA3-SUF **derived-negative**; E8 constructed candidate; E9 **paper-claim locked**. | [PRTOE_CHAIN_TABLES.md](PRTOE_CHAIN_TABLES.md) |
...
**Non-claims:** no physics COMPLETE from this file alone; **not** Page closed; **not** nested ΔlnZ booked; Stage A bbnfix/TRGB posteriors are **not** a decisive evidence win; Strong CP remains **COMPLETE-ABSTENTION** (seat hunt ≠ solution).
