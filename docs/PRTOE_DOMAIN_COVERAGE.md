...
|---|--------|---------|-----------------|-----------|
| 1 | Cosmology (CMB/BAO/SN background) | mover | The model: H₀ ≈ 69.9 (provisional, **not** from live tables), S₈ eased, plik better than ΛCDM's own. The **public hard case** is still the fixed/derived-ε zero-extra-parameter lane **if** the full stack holds; the **current live AWS nested run is not that lane**. It is the softer **sampled-ε** evidence dyad (`cmp_prtoe_dyad_ev`), currently up but watcher-marked **STALLED**, with the repaired ΛCDM twin worker waiting behind it and **no nested verdict bookable yet**. Laplace ΔlnZ = +2.635 remains the graded evidence number until the nested pair finishes. Production bbnfix pair **NOT bookable** as of 2026-08-06 (lcdm R−1 **0.049324** N=26294 t=2026-08-05T11:52:10 — control leg ready, `converged:true`; dyad R−1 **0.085619**@N=27525 t=2026-08-05T22:03:22 — **1.71×** stop, `converged:false`; pair still refused; **no peek H₀**) | the headline-result document (v5_dCDF_complete, archived); [PRTOE_CHAIN_TABLES.md](PRTOE_CHAIN_TABLES.md) |
