...
> weak: sample-covariance Laplace **ΔlnZ ≈ +0.21**, not a nested-quality win. Stage A MCMC is booked on old-BAO SH0ES, DESI-DR2 SH0ES, and DESI-DR2 TRGB (do not mix). Nested sampling is launched (UltraNest + PolyChord). LCDM UltraNest one-legs are finished; dyad legs are not. There is no booked nested ΔlnZ. Do not treat mid-run nested logZ as a result. BBN ε arithmetic remains **verified internal**;
...
| the amplitude / ε | fractional electron-mass shift. Three numbers on purpose: **1.2543%** derived stack (c·f̄·α_c = 27α/5π), **1.232%** production-chain fit, **~1.24%** posterior summary. the 1.8% gap between derived stack and production fit sits inside the posterior width; Stage A MCMC is booked on old-BAO SH0ES, DESI-DR2 SH0ES, and DESI-DR2 TRGB (do not mix); nested twins remain unfinished |
...
- The historical statistical win (ΔlnZ = +2.635) is a **Laplace** estimate, SH0ES-conditional and
  ...
  Laplace only **ΔlnZ ≈ +0.21** with soft modes. Stage A MCMC is booked on old-BAO SH0ES, DESI-DR2 SH0ES, and DESI-DR2 TRGB (do not mix). Nested sampling is launched (UltraNest + PolyChord). LCDM UltraNest one-legs are finished; dyad legs are not. There is no booked nested ΔlnZ. Do not treat mid-run nested logZ as a result.
...
| 4 | ε three values on purpose (1.2543 / 1.232 / ~1.24) | **honest fence** | glossary row | Stage A booked; nested twins unfinished |
