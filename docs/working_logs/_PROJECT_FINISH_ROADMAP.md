...
- Three arXiv packages humanized; clean-room builds; readiness note current
...
|---|---:|---:|---:|---|
| dyad_mnu_bbnfix | ~0.19 | 0.05 | ~6% | improving slowly |
| cmp_lcdm_mnu_bbnfix | ~0.14 | 0.05 | ~9% | closest |
| cmp_prtoe_routeD | ~129 | 0.1 | ~5% | better than 10⁴ era; ranks still partially disjoint; may need another proposal pass |

When dyad + lcdm both hit R−1 ≤ 0.05 (+ CL test): run Laplace / finalize scripts; update
CHAIN_TABLES with bookable GetDist; close T3 / T11 machine halves.
...
| deliverable | status |
|---|---|
| supertrace-note | Zenodo; arXiv optional |
| neutrino-mbb | TeX ready; **hep-ph endorsement** |
| radio-lattice | TeX ready; **astro-ph endorsement** |

## Next recommended sessions

1. Leave bbnfix pair alone; watch R−1 weekly; book when ≤0.05  
2. RouteD: if R−1 plateaus above ~10 with raw accept still ~5%, reseed covmat smaller from
   overlapping best samples (3-rank) — config surgery, not waiting  
