...
| 5 | `papers/bbn-eps-bound/` | `bbn-eps-bound.{pdf,tar.gz}` (+ staged `recompute_eps_bound.py`) | **READY_PACKAGE** | astro-ph.CO | Free ε, Aver Y_p ⇒ **ε < 3.2% (2σ)** at measured T_c ≈ 179 keV for a leptonic m_e turn-on inside BBN window | EMPRESS cannot bound ε (ε=0 already +2.9σ); no D/H derivative bound; no chain-dependent D/H prediction; dense ε_max(T_c) curve **not produced** (optional residual, not a hold) | **astro-ph endorsement** (same as radio-lattice) |
...
*Inventory filed 2026-08-04. No arXiv post. No Fairbank email. No MCMCs.*
