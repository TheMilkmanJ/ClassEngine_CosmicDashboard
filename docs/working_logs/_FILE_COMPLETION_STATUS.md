...
> [`_RESIDUAL_DEBT_CENSUS.md`](_RESIDUAL_DEBT_CENSUS.md) (synced 2026-07-28; table hygiene
> 2026-08-02) and [`_GATED_SHELF.md`](_GATED_SHELF.md) (hygiene 2026-08-02).
>
> **Scope.** The live shelf only (`docs/PRTOE_*.md`). Not `exploratory/`, not `archive/`,
> not `working_logs/` except this log. Status is about the *document* as a forward-facing
> object: whether it has finished its stated job and whether remaining residue is desk,
> machine, theory, or external.

## Status values

| tag | meaning |
|---|---|
| **COMPLETE** | Document has finished its job; residue is optional or already paid |
| **COMPLETE-ABSTENTION** | Finished by deliberately refusing the problem (jurisdiction silence) |
| **COMPLETE-CONDITIONAL** | Finished at its named grade; conditionals are explicit, not papered over |
| **OPEN-DESK** | Residue closable at the desk without MCMC or new mechanisms |
| **OPEN-MACHINE** | Residue waits on a chain, sim, or sampler (not on desk invention) |
| **OPEN-THEORY** | Residue is a real mechanism/derivation (do not mark COMPLETE) |
| **WATCH-EXTERNAL** | Residue waits on external data, facilities, or literature campaigns |
| **LEDGER/HISTORY** | Process, registry, index, or living map — complete as a *record* |

A file may carry a primary tag and a parenthetical secondary (e.g. COMPLETE-CONDITIONAL +
WATCH-EXTERNAL) when both apply; the primary is what the inventory counts.
...
|---|---|---|
| [`PRTOE_INDEX.md`](../PRTOE_INDEX.md) | LEDGER/HISTORY | Live shelf map; exploratory/archive carve-outs dated 2026-07-28 |
| [`PRTOE_READERS_GUIDE.md`](../PRTOE_READERS_GUIDE.md) | COMPLETE | Decoder + reading order; job is orientation, not results |
| [`PRTOE_READERS_RISK.md`](../PRTOE_READERS_RISK.md) | COMPLETE | Outsider risk page; strongest claims graded, kill conditions named |
| [`PRTOE_THREE_EQUATIONS.md`](../PRTOE_THREE_EQUATIONS.md) | COMPLETE-CONDITIONAL | Elevator front door; DE existence claim + lattice referee explicit |
| [`PRTOE_MATH_SPINE.md`](../PRTOE_MATH_SPINE.md) | COMPLETE-CONDITIONAL | Full chain with grades inline; Route-D fork and open items named |
| [`PRTOE_THE_AMPLITUDE.md`](../PRTOE_THE_AMPLITUDE.md) | COMPLETE-CONDITIONAL | ε stack + three referees; production fit provisional on chain hygiene |
| [`PRTOE_DEPENDENCY_TREE.md`](../PRTOE_DEPENDENCY_TREE.md) | LEDGER/HISTORY | Living parent/child map; weakest-parent inheritance rule |
| [`PRTOE_DERIVATION_HUNT.md`](../PRTOE_DERIVATION_HUNT.md) | LEDGER/HISTORY | Working front of underived numbers; not a closed result |
...
| [`PRTOE_me_mechanism_math.md`](../PRTOE_me_mechanism_math.md) | COMPLETE-CONDITIONAL | Equations companion; C_ref input and formation residual named |
| [`PRTOE_bbn_witness.md`](../PRTOE_bbn_witness.md) | COMPLETE-CONDITIONAL | Constraint computed; manuscript-ready status; helium fork external |
| [`PRTOE_deuterium_row.md`](../PRTOE_deuterium_row.md) | WATCH-EXTERNAL | Absolute row awaits LUNA d(d,n)³He (P-2026-058); model−ΛCDM gap not waiting |
| [`PRTOE_fingerprint_lattice.md`](../PRTOE_fingerprint_lattice.md) | COMPLETE-CONDITIONAL | Cross-messenger correlation structure; D/H error budget paid (T13) |
...
| census row | inventory alignment |
|---|---|
| T1 galactic atoms — priced; leading test at pc radii | OPEN-MACHINE |
| T2 SMBH — both #31 halves closed; α_g chain-gated; NewAthena WATCH | OPEN-MACHINE (desk paid; scope line updated 2026-08-02) |
| T3 neutrino home — joint fit MACHINE | OPEN-MACHINE |
| T4 S₈ — matched lensing OPEN; conv_desi unproduced | OPEN-MACHINE |
| T5 low-ℓ — estimator built; data application external | WATCH-EXTERNAL |
| T6 Koide — pacing + sign-chain OPEN | OPEN-THEORY |
...
| T15 indirect — discharged | COMPLETE |
| T16 LSS parity — DESI 4PCF WATCH | WATCH-EXTERNAL |
| Area law regulator + roster | paid → entropy / QG / BH rows COMPLETE(-CONDITIONAL) |
| Bounce / #101–#102 / basement | remain OPEN-THEORY; **not** marked COMPLETE |

## Cross-check vs gated shelf (2026-08-02 hygiene)

| shelf section | inventory alignment |
|---|---|
| §1 lattice note (#67) | WATCH-EXTERNAL on `PRTOE_lattice_note.md` |
| §2 routeD / conv_desi | OPEN-MACHINE on S₈ + calendar (conv_desi unproduced per S₈ files) |
| §2 zon_disp not relaunched | Calendar table corrected 2026-08-02 to match |
| §4 nested evidence / cluster | CODE_MANIFEST + hubble / referee calendar (MACHINE/cluster) |
| §5 sims (#173, granule, χ-lag) | granule_scoping OPEN-MACHINE |
| §6 desk still owed (#101/#102, #115–#117, #130, #146, #161, #22) | OPEN-THEORY / OPEN-MACHINE as above; **not** closed here |

---

## Desk edits this pass (2026-08-02)

1. **`PRTOE_REFEREE_CALENDAR.md`** — "Sitting NOW" table: zon_disp was labeled "relaunched and
   running" while the body and gated shelf §2 say deliberately not relaunched; conv_desi was
   labeled as a live relaunch while S₈ companions record it unproduced after dying twice.
   Rows rewritten to match gated shelf + residual census (no chain-state kill/edit).
2. **`PRTOE_blackholes_no_singularity.md`** — summary table cell "derived, one regulator residue"
   contradicted paid regulator O(1) + roster extension; retagged derived-conditional at candidate
