...
| 149 | THE DEEP AUDIT, checks 12 and 13 | open — **part-paid 2026-07-29** (`_AUDIT_LEDGER.md`, that date). Check 13 run over the seven forward-facing files carrying a literal `Owed` section; three defects fixed, four sections verified already-classified. The find was a dead premise under a live conclusion in `PRTOE_s8_growth.md` — the body corrected on 07-28 to "unproduced rather than pending", the italic line beneath it still reading "the chains testing it are running". **Still open:** check 12 as a whole-file read across the other ~57 forward-facing files, and the 135 owed-class markers with no outcome word — though most of those sit in the failures ledger and the prediction registry, which record what was owed on a date and are carved out for the same reason the rename pass carved them out |
