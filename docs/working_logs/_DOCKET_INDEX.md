...
| 13 | The DISPERSION zon chain: α_c + the two-clock frame + the n-instrument | open — **dead, not relaunched** (R−1 = 23.3; config covmat covers 12/13 parameters and knows nothing about `log10_zon`; restarting reproduces a known failure — gated shelf §2) |
...
| 149 | THE DEEP AUDIT, checks 12 and 13 | open — **part-paid 2026-07-29 + check-12 residue 2026-08-02 + batch continuation 2026-08-02** (`_AUDIT_LEDGER.md`). Check 13: seven literal-`Owed` sections (three defects fixed). Check 12 residue: morning pass 9 files (owed/still-open); **continuation 14 files / ~25 edits on the "running chain" dead premise** (quartet_clock, THE_AMPLITUDE, neutrino_sector, THREE_EQUATIONS nested-sampling-now-executing, CODE_MANIFEST live zero-param test, fairbank basin "today", honest_status 07-20 snapshot, CHAIN_TABLES offline column, etc.). Calendar Sitting NOW already current. **Still open on #149:** whole-file deep reads of remaining unread giants (shared with #94; hierarchy queued), and the 135 bare owed-class markers inside the failures ledger + prediction registry (carved out as historical record) |
