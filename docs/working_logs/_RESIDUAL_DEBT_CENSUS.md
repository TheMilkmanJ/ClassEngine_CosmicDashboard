...
| **T2 SMBH atoms** | superradiance prong audited (blackholes file) | α_c posterior → α_g | BH spin-mass data | **the window is COMPUTED (2×10⁸–3×10⁹ M☉ spin-down band) and the spin-mass confrontation is now a live exposure** — high measured spins populate the band; the quartic's saturation corrections remain |
...
| **T5 low-ℓ** | matched-circles; **the cavity C_ℓ computation — RUN 2026-07-18: right shape, 3–5× too little depth at permitted sizes (ledger re-grade)** | — | BipoSH (analysis-limited) | all three escapes computed and dead; **the whole power-spectrum route is ungradeable** (total S/N **0.16** over ℓ = 2–6, depth and shape alike) — the test moved to the off-diagonal correlation structure, regenerated ISW-inclusive on a retained script (**S/N 1.4**, retention 90% at the floor; `scripts/torus_lowell_pattern.py`). The pattern is on the calendar for BipoSH, which now grades 1.4σ rather than the 2.2σ the first pass reported |
...
| **T7 lab cousins** | — | — | ring-BEC literature | **specified 2026-07-18** (three proposals with numbers + the mapping table; the quench test is already run in the literature and corroborates the machinery, not the amplitude) — what remains is whether an existing dataset already measures the winding average |
| **T9 direct detection** | item 1 (σ ≈ 8×10⁻¹⁵⁰ cm²) | — | the neutrino-fog calendar | the haloscope edge cases |
