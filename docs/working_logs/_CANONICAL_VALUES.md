...
| **the ε-blind ensemble** | **c = 0.903 ± 0.0375** — a *check* on c | a selection among the candidates | It lands −0.08σ from 9/10, and at the same width sits +0.53σ from 12/13 and −0.38σ from the charge²-weighted **8/9**, which is only **0.30σ** from 9/10. The figure **σ_c ≤ 0.0115** is the candidate *spacing* (3.3× relative to the present width), not an exclusion width: it buys a 0.97σ separation from 8/9, where the two stop coinciding, not where either is excluded. A 3σ call on the binding pair needs **σ_c ≤ 0.0037** — 10× in width, ~100× in sample (`scripts/c_census_discrimination_width.py`). What picks 9/10 over 12/13 is the tie-as-lock argument, not this number (#126). |
