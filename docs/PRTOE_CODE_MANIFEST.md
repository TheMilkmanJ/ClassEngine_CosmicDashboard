...
## 1. IN — running now (the arbiter's own physics)
...
chart (Γ/impulse/E/friction); the Widnall n-predictor (n = 2.26–2.51 × R/a — zon/comb
...
they do not alter any observable CLASS computes (the sweep-55 energy nulls are the
...
environmental knobs (census-illegal); the sound-horizon lock (+6 blue); the μ-era H0
lever (×17–83 short); the S8-flow lever (energy null + wrong-signed); the burst
...
Every [C]-expressible beneficial item is ALREADY COMPILED into the live .so the arbiter
