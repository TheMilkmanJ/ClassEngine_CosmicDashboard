...
| B3 | **the k_int O(1) audit** (referee 1's residue) — *superseded 2026-07-19: k is now derived outright from the two-band screened particle-hole kernel, hierarchy §6c/§6e, and confirmed by Monte-Carlo* | the interaction integral's surface-DOS + normalization conventions, forced from the roster | the Eliashberg kill window (k ∈ [1.35, 1.37]) | done |
