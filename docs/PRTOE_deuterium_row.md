...
so "the new physics beats ΛCDM on deuterium" is not a claim available to it. What the counterfactual
...
**This is the sharpest experimental ask the row generates**, and it is not a request for new
...
Priced at the amplitude the cure needs: ΔN_eff = 0.49 below T_c zeroes deuterium for ΔY_p = 0.0020,
...
structural one; it stays closed until the data moves.
...
The inventory leaves exactly one shape of object unaccounted for:
...
**The three seats that do exist are too light.** At the MeV-scale point of v_L the seesaw puts the
...
on the universe's expansion, and both are therefore non-gravitational in character. The row is the
...
| the scalar → lepton loop → 2γ → quark bridge | **too weak** — two EW/EM loops, 20 000–31 000× short of the lever it would need | **+8×10⁻⁵σ** — nonzero, and four orders under the smallest term above |
...
| ν_R at the MeV-scale point (~4.2 MeV) | below the ⁴He threshold; a threshold does not partly fire |
...
[PRTOE_neutrino_sector.md](PRTOE_neutrino_sector.md) (the Majoron parameter points),
...
**Non-claims:** not a BBN win; NACRE II not a heal; no free nuclear dial.

**Triage:** elevate-in-place. Physics ceiling: owned-adverse machine; absolute row **OPEN-BLOCKED** (LUNA).
