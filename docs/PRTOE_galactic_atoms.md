# Galactic Atoms — Every Galaxy Has a Ground State (2026-07-11)

*The atom grammar applied to galactic dynamics, with the model's banked mass. Status:
STRUCTURAL (candidate) with computed numbers; booked def419. The improvement over standard
CDM: a predicted, computable ground-state object in every halo — and an explanation, rather
than an accident, for why this model SURVIVED the constraints that killed lighter fuzzy DM.*

## 0. The grammar

If the dark matter is a condensate of quanta with mass m, every galaxy is literally a
**gravitational atom**: a self-bound quantum system with a ground state (the central soliton —
"the 1s orbital"), an excited spectrum (the granule/halo structure), and a binding scale set by
the gravitational coupling. This is standard fuzzy-DM structure — the model's additions are
(i) the mass is BANKED, not tuned per-galaxy (m = 2.24×10⁻²⁰ eV, the H=m onset value now being
MEASURED by the zon chain), and (ii) the grammar is the same one that derived the cosmic floor
(the Bohr×BEC skeleton of def337).

## 1. The ground state, computed

The soliton ("1s") core radius scales as r_c ∝ 1/m² per halo mass; with the banked m:

    dwarf halo (10⁹ M☉):    r_1s ~ 7×10⁻³ pc  (~1400 AU)
    MW-class (10¹² M☉):     r_1s ~ 7×10⁻⁴ pc  (~140 AU)

**Sub-parsec ground states.** Two consequences, both improvements:

1. **Why this model survived where fuzzy DM died (banked, now explained):** the classic FDM
   sweet spot (m ~ 10⁻²²  eV) puts kpc-scale solitons in dwarfs — and was squeezed out by
   Lyman-α and rotation-curve data (the banked inherited verdicts). At the model's 224×
   heavier mass, the 1s orbital is BURIED at sub-pc scales: halos look CDM-like everywhere
   data currently reaches. The survival was not luck; it is the level structure.
2. **A new prediction class (flagged, un-priced):** every galactic nucleus should host a dark
   soliton of computable radius and mass — a smooth, dissipationless sub-pc mass component
   coexisting with the nuclear star cluster and the central BH. Its dynamical imprint
   (millipc-to-pc stellar kinematics, e.g. S-star-cluster residuals in the Galactic Center)
   is the domain's honest test — OWED a pricing pass before any registration.

## 2. The excited spectrum

The halo above the soliton is the atom's excited/quasi-thermal spectrum: granule interference
(the banked granule structure, S = (1+(1−f_amp)²)/2 — note f_amp enters HERE too: the same
partition measured by the ramp), relaxation by wave interference rather than two-body
scattering. Standard heavy-FDM phenomenology inherited; the model's specific f_amp value is a
cross-link no generic FDM has.

## 3. Honest scope

- The soliton scalings are the standard Schive-class relations at OOM precision; the model's
  content is the FIXED m (one number for every galaxy — falsifiable in aggregate).
- The zon chain's mass posterior will TIGHTEN or MOVE every number in §1 — this document is
  m-parametric by design.
- No conflict with current data is known (that is the survival statement); the improvement is
  explanatory + one flagged predictive prong.

*Galaxies are not piles of dark gravel. They are atoms too large to see whole — ground states
buried under starlight, spectra spread across kiloparsecs, all sharing one electron mass
written at genesis.*
