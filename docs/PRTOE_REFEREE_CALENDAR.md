...
| **zon_disp — relaunched and running 2026-07-20** (the collapsed R−1 = 23.3 run is archived at `chains/_archive_zon_disp_collapsed_20260720_1528/`) | P-040 (α_c = 3α), the triangle (M₂, x₀, ρ_Λ), the pair mark, the n-instrument, FIVE freezes | converged (R−1 < 0.05) center INSIDE 7.4–7.7 = 3α-compatible (bath band); at ~7.55 = clean confirm; ABOVE ~7.8 = the named branches must pay | burn-in, no ETA yet |
...
| **dyad_mnu — the control case, and it is not explained** | (not a referee; recorded because it constrains how the others must be read) | — | Converged: **R−1 = 0.176 at N = 8736**, the healthiest chain on the box, and it breaks the tidy story about the rest. Its acceptance is **0.92** — as high as the collapsed chains — so a high acceptance rate is *not* by itself the pathology, exactly as `oversample_power = 0.4` predicts for a chain whose fast nuisance steps dominate that column. And it runs **no seeded covmat with default learn gates**, the very configuration that failed elsewhere, on the same 13 parameters. Three candidate discriminators were tested and all three fail. **Not the parameter count** (13 either way). **Not `dcdf_conv_g`** — zon and zon_disp omit it and still sat at R−1 = 40.4 and 23.3. **Not the chain count** — every one of the five ran as a single chain, so R−1 is computed the same way for all of them and the statistic is comparable. What has *not* been tested, and is the next thing to look at, is whether the failing chains sample directions the data barely constrains (the zon and dCDF parameters) while dyad_mnu's `varying_me` is tightly held by the CMB: a near-flat posterior direction is explored slowly by any random-walk Metropolis, however well its step size is tuned, and that would be a property of the parameter set rather than of the sampler. **TESTED 2026-07-29 (`scripts/flat_direction_convergence_test.py`), and the hypothesis is UNSUPPORTED — though not refuted.** Posterior width ÷ prior width was measured for all 13 sampled parameters of all four chains. **No chain carries a flat physics direction at all** (every physics r < 0.14 in the stuck chains). The named suspects behave *opposite* to the prediction: `dcdf_rho_inf` is the **narrowest** direction in every one of the three stuck chains (r = 0.010, 0.012, 0.012) and `log10_zon` sits mid-pack. The *shape* is wrong too — a chain crawling along a genuine flat valley should be **wide** in that direction and narrow elsewhere, whereas the stuck chains are narrow in **every** direction, uniformly ~3.5× narrower than the converged control. That is the signature of a chain not moving at all, not one wandering a level valley. **Why this is not a refutation, stated because it decides how much weight it carries:** the measurement compares widths, not mixing, and a stuck chain understates its own widths — a bias measured here at **3.5×**, larger than any plausible signal, and pointing the same way as the result. A chain stuck badly enough would fail to explore even a genuinely flat direction, producing exactly this picture. So the *named suspects are excluded* and the general claim *survives as untested*. Settling it needs the likelihood's curvature at the best fit (a property of the posterior, not of any chain's wandering) or a short well-tuned run on the same parameter set — real work, not attempted. One incidental finding worth keeping: this test's first control failed because it assumed the tightly-priored calibration nuisances should come out *constrained*; for a nuisance **the prior is the constraint by design**, so r → 1 is correct behaviour, and "flat relative to its prior" is meaningless for such a parameter. Nuisances are therefore excluded from the count, or it measures the prior schedule rather than the data. The sharp comparison is zon_disp at N = 6912 → R−1 = 23.3 against dyad_mnu at N = 8736 → R−1 = 0.176: similar length, opposite outcome, **cause unknown**. Consequence for the two relaunches: the seeded covariance is established to enlarge the step size (routeD, 0.99 → 0.306, measured from the launchlog) but is **not yet established to deliver convergence**. That claim waits on routeD's own R−1 and must not be assumed from the acceptance number alone |
...
>
> It settles the step: the proposal is now the right size
