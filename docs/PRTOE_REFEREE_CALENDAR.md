...
> **Present instruments.** Old-BAO + DESI-DR2 SH0ES + DESI-DR2 TRGB Stage A **booked**. On SH0ES, nested sampling got close enough to compare the twins, not far enough to book a full sampler. No nested ΔlnZ. α_c retune **stopped**; GetDist done; `log10_zon` **inconclusive** (7.57±0.51). Progress-file `acceptance_rate` is oversampled (`oversample_power = 0.4`) — use **raw accept = accepted/steps from launchlog**.
>
...
> On SH0ES, nested sampling got close enough to compare the twins, not far enough to book a full sampler. No nested ΔlnZ. zon retune **STOPPED** (GetDist inconclusive); conv_desi retune **STOPPED** (`g` inconclusive).
...
| The nested referee — DESI-DR2 dual UN+PC GIL (+ no-H0 + TRGB) | P-2026-044-style evidence on DESI-DR2; all three anchors | compare dyad vs ΛCDM **within the same ladder/no-H0 anchor** only; no mixed-anchor ΔlnZ; no verdict until final summary JSON | On SH0ES, nested sampling got close enough to compare the twins, not far enough to book a full sampler. No nested ΔlnZ. ETA stamps: `nested_pc_eta_20260815`. |
...
> leave two cores free). **Live path (2026-09-02):** `cmp_prtoe_zon_disp_retune` **STOPPED** (R−1=0.036, `converged: true`); GetDist **INCONCLUSIVE** `log10_zon = 7.571 ± 0.511`. Not α_c = 3α (Sitting NOW row above).
>
...
> the current nested referee: On SH0ES, nested sampling got close enough to compare the twins, not far enough to book a full sampler. No nested ΔlnZ. **Stage A posteriors are booked**; nested ΔlnZ is still on the critical path for P-2026-044.
...
| The zero-parameter evidence exposure — currently carried by Laplace-from-MCMC while on SH0ES, nested sampling got close enough to compare the twins, not far enough to book a full sampler. No nested ΔlnZ | ε, A_s and n_s stated vs ΛCDM free — **z_on excepted**, frozen 0.053 dex off the onset identity | ΔlnZ verdict; any stated number wrong collapses the model's evidence. The booked old-BAO pair carries only an inconclusive sample-covariance Laplace **ΔlnZ ≈ +0.21** with soft-mode sensitivity. Until a finished nested twin exists, the exposure is real but the reading remains marginal |
