...
| **the thaw chain — `cmp_prtoe_routeD`** (samples `dcdf_floor_thaw` = 1+w_fl,0); collapsed run archived 2026-07-20; fifth launch is the live object (do not kill) | thaw = 0 (the no-bare clause's direct falsifier) | posterior excluding 0 = evidence against the clause itself | **MACHINE** — live under owner policy; R−1 not at target. Collapsed first launch (R−1 = 24.89, acceptance → 0.99) archived at `chains/_archive_routeD_collapsed_20260720_0639/`. Seeded-covariance repair measured (acceptance 0.99 → ~0.25–0.31). Read the *live* chain's own files before grading — archive paths still carry dead run products |
