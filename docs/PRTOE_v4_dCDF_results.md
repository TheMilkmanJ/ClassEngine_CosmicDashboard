...
|---|---|---|
| ΛCDM anchor (H0=67.4, fiducial) | 2515.45 | — |
| dCDF hand-tuned (H0=69, ξ=0.15, n_s=0.98) | 2511.57 | −3.9 |
| dCDF CosmicForge best fit (H0=70.40) | **2496.24** | **−19.2** |

Best fit: ω_b=0.02280, H0=70.40, logA=3.068, n_s=0.9826, z_reio=8.13,
...
The model buys ~3 km/s/Mpc of H0 and a slightly lower S8 at almost no CMB
cost. A second multistart mode at H0=69.44 (ξ=0.240) sits Δχ²=1.1 away —
the H0–ξ degeneracy valley is flat between at least 69.4 and 70.4.
...
2. **Acoustic-scale locking.** The unified fluid + ξ_Neff shift both r_s and
   ...
   The viable region is the θ*-locked contour in (H0, ξ): roughly
   ξ ≈ 0.15 at H0=69 → 0.40 at H0=70.4 → 0.60 at 71 → 1.0 at 73.
   Off-contour points are catastrophic (0.9% θ* offset ≈ +1600 on plik).

3. **Damping tail prices the H0 relief.** Along the contour, fixed-fiducial
   totals: H0=69 → +11.6; 71 → +228; 73 → +759; 74.8 → +1418 (vs ΛCDM).
   The n_s–N_eff degeneracy absorbs part (optimum n_s ≈ 0.98–0.995;
   ω_b wants to stay at 0.0224). SN gains saturate at −40 by H0≈73.
   Net optimum: H0 ≈ 70–70.5.
...
- **β→0 null test: PASSED.** At β=0 the background is exactly ΛCDM
  ...
  the ΛCDM anchor to σ8 5-decimal agreement and Δχ²=1.4 (residual: β=1e-12,
...
- The null test caught a real bug on first run: **budget overclosure**
  (configs passed Omega0_dcdf = 1−Ω_b, ignoring Ω_r+Ω_ncdm ≈ 1.5e-3 → +20
...
| precision stability (tight tols) | PASS — σ8 stable to 5e-6 |
...
- **β is unused** (best fit 10⁻⁸, i.e. the exact-ΛCDM-background limit of the
  fluid). The H0 relief comes from ξ_Neff + the fluid's budget unification,
...
- S8 = 0.826 at best fit (mode 2: 0.813) vs ΛCDM 0.833 — mild easing, not a
...
- Evidence (ln Z) comparison vs ΛCDM: pending (CosmicForge bridge sampling,
  ...
  ΛCDM samples 7 (ω_b, ω_cdm, H0, logA, n_s, z_reio, A_planck), dCDF samples
...
- H0=70.4 closes ~55% of the gap to SHOES (74.0±1.0); Pantheon+SHOES χ²
  improves −27.8, so the SN data endorse the partial move. Full 73+ remains
  blocked by the damping tail (ξ≈1.0 territory).

## Reproduction

- Best-fit configs and the θ*/n_s/ω_b grids: Claude job f9b88ca9 scratch
