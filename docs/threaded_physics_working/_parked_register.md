# The Parked Register — snagged, analyzed, set aside (per the operator's law)
...
|---|---|---|
| H-TOE (the maximal reading) | Gates un-paid; t470's adverse census forecast (proper N ~ 8-11 vs the rigid 13) | Gate 2's careful signed count; the basement compute |
...
| the double-ladder alignments (web/mesoscale) | loose coincidences, no mechanism (t456) | a derivation linking ξ_K or 1/M₂ to the matter transition |
| the generations lane (3 cycles = 3 families) | a 3=3 match without a mechanism (t460) | the flavor-from-winding construction |
...
| the mass-top triple coincidence (α_g=1 / r_s=ξ / CSW-max) | m-correlated, noted-only (t450) | the zon posterior + an SMBH mass-function mechanism |
...
## THE CROSS-COMPARISON PASS (operator-directed, def489): each parked item vs all domains
...
| mass-top triple | SMBH demographics = the fabric; needs zon + a mechanism | parked unchanged |
