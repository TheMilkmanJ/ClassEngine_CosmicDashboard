...
1. THE SIGN MAP (load-bearing): the family predicts sign(helicity_B) = sign(n) = the matter draw — but WHICH handedness corresponds to matter-wins runs through Card 4's rectification mechanism (owed there). Until the map is explicit, the measured hint cannot be read.
...
the chain into the cross-messenger test (measured helicity ⟹ predicted baryon sign ⟹
checked against the fact that matter won). When Card 4 lands, T14 item 1 closes and the
