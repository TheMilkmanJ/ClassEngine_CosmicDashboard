# PRTOE — UV Completion for Ψ (the working docket): deriving ξ/c
...
**ρ = -P(X0) = const, w = P/(-P) = -1 exactly.** That IS the floor. "Funded" = energy from the
kinetic condensate (not a bare Λ); "sign-locked" = stability needs P''>0 (floor, not ceiling).
...
cancellation rather than by a see-saw between a UV and an IR scale. Three recorded readings, and
every one of them lands inside the escape window:
