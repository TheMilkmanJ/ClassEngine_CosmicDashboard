> **PROVENANCE NOTE (2026-07-12):** skeleton/outline era; for current state see PRTOE_INDEX.md.

# PRTOE: The Skeleton in Plain Math (2026-07-07)

*The core structure, stripped to its hinges. Not a sum -- a SEQUENCE, each step
feeding the next. Written after the operator assembled the ingredients (R=0,
f_rot, Psi_0) and asked how they fit.*

## Step 0 -- the blank ledger
    R = 0
Empty spacetime is flat; nothing happens (the equation-of-nothing / "nothing
until contact"). DERIVED (bedrock GR).

## Step 1 -- the first transaction: the medium is born (genesis)
The medium is a complex field Psi = |Psi| e^{i xi} in the potential
    V(Psi) = m^2 |Psi|^2 + lambda |Psi|^4
The quartic term lambda|Psi|^4 is the medium meeting ITSELF (the only thing to
transact with at t~0). This genesis (Affleck-Dine, the quartic era) runs ONCE
and produces TWO numbers:
    Psi_0   = the field strength / abundance   (the "weird symbol")
    f_rot   = the rotor fraction = the psi/chi two-fluid split (the charge)
STATUS: the mechanism is DERIVED (Room 1); the exact Psi_0 and the f_rot
distribution come from solving the quartic-era dynamics (the dice, One Mountain).

## Step 2 -- the account opens: R stops being 0
Now Psi exists and has energy, so it curves spacetime:
    G_{mu nu} = 8 pi G  T_{mu nu}[Psi]
This is the medium "investing into the universe" -- dark matter's gravity and
dark energy's push, ONE field both effects (dCDF, the unified dark fluid).
DERIVED + FITS the data (H0=69.7, S8 eased; the "real-fit").

## Step 3 -- the two checkable outputs
    m_e shift:   eps ~ Psi_0 / M_Pl  ~ 1%     (set by Psi_0)
    granule:     S = (1 + f_rot^2)/2          (set by f_rot; CONFIRMED numerically)

## The hinges the operator named
    R=0     -- the blank start (Step 0)
    Psi_0   -- one genesis output -> the m_e amplitude (Step 3)
    f_rot   -- the other genesis output -> the granule meter (Step 3)
They don't ADD (different kinds of quantity); they are the START and the TWO
CHILDREN of the genesis. The genesis (Step 1) is the "multiplication table" where
Psi_0 and f_rot are computed -- the one box not fully solved (solving it pins the
exact eps=1.24% and the exact dice).

## Honest status of each piece
  R=0 (Step 0):                 DERIVED (bedrock).
  genesis mechanism (Step 1):   DERIVED (Room 1 identity); exact Psi_0/f_rot OPEN.
  medium sources gravity (2):   DERIVED + FITS (real-fit, unconfirmed).
  eps ~ Psi_0/M_Pl (Step 3):    SIZE derived (~1%, two routes); exact value OPEN.
  S=(1+f_rot^2)/2 (Step 3):     CONFIRMED numerically (<1%); dynamical sim OPEN.
