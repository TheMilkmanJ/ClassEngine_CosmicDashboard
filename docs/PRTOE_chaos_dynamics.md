...
- the release-angle chaos: f_amp(θ_i) spans [0.034, 0.964] with adjacent-bin jumps (internal record 335 —
  the recorded dice made quantitative); the cycle map: NO attractor (7 chains × 120 cycles,
  spreads [0.1, 0.97] to the last cycle — internal record 361)
- the resolution: TOPOLOGY averages what chaos scatters — f̄ = ∫f_amp(θ)dθ/(π/4) = 0.635 ±
  0.026 (ramp-corrected, internal record 407): the winding's uniform measure integrates the chaos out
- the odometer: f̄(Ri) shallow (~0.09 per 100× energy) — chaos-robust, clock-weak (internal record 387)
