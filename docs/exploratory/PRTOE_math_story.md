 ...
 **naturalness ban is dormant while m_eff ≈ 0** → direct coupling ALLOWED
 The epoch structure is recorded (the window closes at basin entry, and that closure is
 load-bearing); every magnitude so far proposed to run through it has died — the one-carrier
 κ_v by eight orders (ANN-2026-006), the flat vev at +7.7σ — so nothing standing rides this
 ...
 **δm_e/m_e = ε · [Θ unsaturated]** — and the gate is a near-step, not a Gaussian:
 ...
 excluded if "everywhere" (methanol maser 9×) → screening is data-required;
