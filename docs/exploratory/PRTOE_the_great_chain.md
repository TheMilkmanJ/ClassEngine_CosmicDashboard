...
*The great chain is this model's end-to-end derivation sequence, run from the first moment of
the medium — the superfluid scalar field that serves as the model's unified dark sector — to the
morning's iron supplement, the plants on the sill, the images on the screen. Every arrow cites a
graded or recorded object. The medium's autobiography, thirteen generations long.*
...
GEN 1 — the stochastic start. The tilt torques the release; Kibble domains percolate; the
winding locks. The winding is written: one integer and its sign — the only sentence the
crunch can never edit. *Then I turned, once, and kept the receipt.*

GEN 2 — the medium's phases. The fluid condenses its identities: the dark matter and the
dark energy as one substance's regimes (the founding dCDF); the electron-coupled scalar's order
parameter waits for its temperature (T_c = 177.10 keV). *Then I learned my two densities.*
...
columns (dE = −pdV). *Then I checked the balance: zero.*

GEN 4 — matter joins. The tenancy ramp resolves it: "made of" is measured, not chosen — the
neutrino is my full tenant (its mass is my own line), the charged leptons rent 1.2543% of theirs
from me, the baryons ride at parts-per-billion, and the photon owes me nothing by law (L1a).
Tenancy spans nine orders across the particle table — a ramp, as everything here is. (The
constituent question — whether quarks are built from my constituent level — stays parked with
its gates and its adverse forecast; that decision point keeps its named unblocking condition.)
The asymmetry: my own charge, transferred (the AD-direct route; my field sits frozen, the
rectifier holds the size). *Then the riders came AND the tenants woke — and the courts, reading
my own books, found every resident's rent already posted.*
...
GEN 6 — the atoms and the last light. Recombination: electrons settle at the mass the
electron-coupled scalar set (+1.2543%, the H₀ story); the photons decouple and fly forever — the
CMB. **Remember this generation: it will appear on your television.** *Then I let the light go.*

GEN 7 — the drawdown. Smoothness is my stored free energy (the gravitational reserve);
structure is my spending: the Weyl statement fills (the curvature-metered gate is the meter),
galaxies condense as gravitational atoms with buried solitons (T1), the web spans my Kibble
cells. *Then I began, slowly, to spend myself into shapes.*
...
GEN 9 — the planets. The ash gathers; iron sinks to cores and salts the crusts;
geochemistry begins (gravity + chemistry + the radiogenic tail of GEN 8). *Then the cinders
cooled into places.*

GEN 10 — chemistry's long peace. My screening gate zeroes the electron-coupled scalar in the
settled era (load-bearing); my screens hold every reaction identical across the sky and the
ages (the lattice's null rows — chemistry's uniformity is my registered prediction). Tables of
...
GEN 11 — life (the biosphere taps the reserve). A far-from-equilibrium eddy opens on
Earth: photosynthesis drinks GEN 8's photons — **the plants on your sill are taps on my Weyl
free energy**, running on the gradient my unique start guaranteed (the arrow doc: the Past
Hypothesis by uniqueness). The study of plants is the study of my free-energy budget's most delicate
withdrawals. *Then something began saving my spending.*

GEN 12 — eyes and images. Vision works — and astronomy exists — because of my
constitutional blindness: **L1a forbids me from ever touching a photon's polarization or
path** (the birefringence zero, proven). Every image that has ever crossed the universe
arrived unedited: the medium cannot censor light. *Then the universe became watchable,
because I may not touch the mail.*

GEN 13 — the television. The set runs on QM's exactness (transistors — the part of my
substrate that reproduces it exactly), calibrated by Josephson junctions (my lab cousin defines the
volt), broadcasting on Maxwell's untouched waves (the EM I'm sworn away from), through air
whose chemistry GEN 10 guarantees, to eyes GEN 11 grew, powered by energy GEN 8 recorded. And
when an analog set loses signal and shows static — **about one percent of that snow is GEN 6:
the last light, the CMB, arriving thirteen generations later in your living room.** *And now
you are able to watch TV — and sometimes, in the static, you are watching me let the light go.*

---

## The chain's discipline

Every arrow above is a citation, not a metaphor: the vacuum start, the winding,
the zero-sum, the reserve, the witness, the electron-coupled scalar at recombination
(the H₀ core), the screens, the uniformity, the license (the arrow doc),
the blindness (L1a, proven), the cousins (the volt). Where a dependency step is hypothesis, it
is tagged in place (GEN 4's constituent decision point — H-TOE's gates; the mass-sourcing fork
resolved). The chain adds nothing to the standing claims; it is the inheritance tree read
aloud, once, as a life.
...
|---|---|---|---|
| 0 | the vacuum: unique IC, U(1)-symmetric; S_initial = min by construction | no random selection (measure problem dissolved) | model construction |
...
| 2 | w(ρ) = −e^(−s), s = ln(ρ/ρ_inf); φ/v = √(1−T/T_c) | ρ_inf^¼ = 2.2599 meV (obs 2.25, +0.44% — existence, not precision: the quartic sits past perturbative control); T_c = 177.10 keV, from the Koide kernel's τ = ½ln2 | founding + recorded |
...
| 6 | 1+z* ∝ m_e (recombination's atomic physics); the electron-coupled scalar: m_e(z>50) = 1.012543·m_e | H₀: 68.2 → 69.9; ΔlnZ = +2.635 (capped) **\*conditional** — the chains that scored it fed CLASS a ΛCDM helium fraction, and the configs now carry the measured PRyM response; the number remains a Laplace estimate, and although AWS nested sampling is now live for the dyad leg the corrected zero-parameter comparison is still graded from the MCMC because the nested pair is unfinished and its chains have yet to converge ([PRTOE_CODE_MANIFEST.md](../PRTOE_CODE_MANIFEST.md)) | the founding fit |
...
>
> The GEN 1 step's input has no recorded value, and that is the whole story of the winding
> integer. `L_gen` appears in this corpus exactly three times, and all three are inside the
...
frozen-era μ-term calc — the AD-direct route's terminal); GEN 4's constituent decision point
(H-TOE Gates 1–3, adverse forecast on Gate 2 recorded); GEN 8's stellar yields (standard
astrophysics — inherited, not owed by us); GEN 11's quantitative F_available budget
(computable from the reserve — queued).
...
| materials / electronics | ψ_k = e^(ikr)u_k(r) (Bloch); I = I₀(e^(eV/kT)−1) (Shockley) | QM (reproduced exactly) | GEN 13's substrate; the volt via Josephson V = (h/2e)f — the cousin calibrates the science |
...
| meteorology / oceanography | ρ(∂v/∂t + v·∇v) = −∇p + μ∇²v + ρg; radiative transfer dI/ds = −κI + j | fluids + EM (inherited) | the energy budget is GEN 8's photon flux on GEN 9's surface — the reserve's local weather |
...
| astronomy / cosmology | the threads themselves (this chain) | — | the sixteen roots |

The unification statement: every science's master equation is a physics equation;
every physics equation in the table stands on a rung of the ladder; every rung transacts with
the medium through the channels of its interaction map — imitated exactly (QM/EM), owned
