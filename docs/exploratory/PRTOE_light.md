...
## 1. The question the photon never gets asked

Every field theory textbook introduces the photon by *postulate*: a massless U(1) gauge
boson with coupling α ≈ 1/137, both properties stapled in by hand. The Standard Model
derives neither. Why is there a massless mode at all? Why does it couple at that strength?
Why does it — alone among the forces' carriers besides the graviton — travel forever,
unconfined, at exactly the limiting speed? The postulate answers none of this; it only
names it.

A medium theory does not get to postulate. If the vacuum is a condensate, every mode it
carries must be *earned* — and the photon must be one of its excitations, with every
property derived or the model dies. This file records how much of the photon this model
has earned, and prices exactly what remains.

## 2. What light is here: the medium's collective mode

The founding identity (the condensate vacuum) implies light is not a resident *of* the
medium the way matter is — it is a motion *of* the medium itself, the transverse
collective mode of the condensate. Three consequences follow immediately, and none of
them is optional:
...
- **Light and the constituent level are the two ends of one spectrum.** The medium's
 excitations
 span the entire range of possible masses: the massless collective mode at the bottom,
 the Planck-mass constituents at the top (the induced-gravity demand forces those
 constituents to 1–2 M_Pl), and every quark, lepton, and atom filling the
 ...
 lightest thing the medium does; its constituent level is the heaviest thing it is.
...
the interaction's information outward without participating in what was built. This role
is a **theorem — the sign structure of the beta functions**:
...
- **A non-abelian coupling grows on descent.** Where it turns strong it confines, and the
 ...
 Λ = M_Pl·exp(−2π/(b·α₃(M_Pl))). Walked down from the Planck-scale boundary value
 ...
 shift in that boundary value rescales all matter by ~×100.
...
Running hypercharge from its measured value at the electroweak scale up to the Planck
scale (one-loop SM, b_Y = 41/6, from 1/α_Y(M_Z) = 98.4):
...
1/α_Y at the Planck scale is **not zero**. The scheme-independent statement:
**gravity's bare requirement is exactly zero (Pauli); light's is 1/α_Y(M_Pl) = 55.5 ≠ 0** —
the constituent level must supply it bare. (Read at M_Z, the medium's loop accounts for 42.9
...
If the constituent level supplies light's bare coupling, it supplies the other forces' too. Running
...
| at the Planck scale (M_Pl) | 1/α (GUT-normalised for α₁) |
|---|---|
| strong | 52.4 |
| weak | 49.4 |
| **U(1) — light** | **33.3** |

- **The two non-abelian couplings NEARLY meet at the Planck scale** — the gap at M_Pl is 2.9
 against a ±2 scheme band, with the exact crossing 2.1 decades below M_Pl; ramped over the
 measurement and scheme bands, only ~2% of the distribution closes the gap at M_Pl. Graded
 honestly: *suggestive, not established* — exact closure would require
 threshold corrections from the constituent theory itself, which is a cost that theory owes
 rather than a result the model holds.
- **The abelian coupling is the outlier** — sixteen away, joining the pair nowhere near
 the Planck scale (the Standard Model's famous near-miss unification, read here from the
 medium's side).

**The constituent level supplies two numbers, not one and not three: one shared by the
...
- the remaining **76.5%** of the electromagnetic coupling. The constituent level must supply the
 ...
 hypercharge channel alone the corresponding unsupplied share is 56% (1/α_Y(M_Pl) = 55.5 against
 1/α_Y(M_Z) = 98.4, so the run supplies 42.9 and the bare value 55.5 is what is owed).

**Both figures are the same debt read in two channels.** What is unsupplied in each is the *bare
Planck-scale coupling*, not any part of the running, which is computed. The electromagnetic 76.5%
and the hypercharge 56% therefore do not represent two separate gaps to be closed independently:
supplying the constituent structure fixes the bare values in both channels at once, and nothing
short of it moves either.
...
parametrization uses); (iv) 1/α₁ ÷ 1/α₂ = 0.673 at M_Pl (equivalently, sin²θ_W at that
scale is fixed by the runs above); (v) the bare coupling α_Y(M_Pl) = 0.0180;
...
Six constraints, one decisive test — plus a further question: whether the constituent
theory's own
thresholds close the 2.9 gap and make the near-meeting (§5) exact. The day a specified
constituent theory satisfies all six, light's last number is derived and this file's
scoreboard closes.
...
constituent theory that fails any of the six constraints** in §6 kills the induced-light
route (and the model's constituent program with it). (iv) The file dies trivially with M3:
no medium, no mode.

*The photon spent a century as physics' most familiar stranger: the particle every
instrument is made of and no theory explains. Here it is the one excitation the medium
cannot help but have — the ripple that rides its own definition of speed, forbidden by
...
(m_γ bound; the measured couplings at M_Z); [Bjorken 1963] (induced/composite gauge
bosons); [Volovik 2003] (*The Universe in a Helium Droplet* — emergent gauge fields at
Fermi points); [Georgi–Quinn–Weinberg 1974] (coupling running to high scales; the
