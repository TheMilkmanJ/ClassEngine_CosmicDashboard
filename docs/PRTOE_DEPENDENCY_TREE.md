...
| the Koide complex (A = √2 ⟸ sector-equipartition) | the equivalence stack stands (A = √2 ⟺ sector-equipartition ⟺ R_c = M_c — mathematics); **the exhibited deliverer (ring-on-ring cascade) is DEAD — executed by its own sealed trial, all three Widnall points in the death zone (ledger, 2026-07-18)** | [OPEN] — A = √2 reverts to an underived sharp number behind the corpus's densest fence network; the protection (multiplicative universality) stands separately | any future mechanism must deliver sector-equipartition while evading the mapped minefield (per-mode/iid/isotropic/face-democratic/geometric-degenerate → 2; six selector graves; the share-entropy class) |
