...
> On SH0ES, nested sampling got close enough to compare the twins, not far enough to book a full sampler. No nested ΔlnZ. Cross-links:
...
Provisional fit diagnostics (sound-horizon driven; SH0ES included though that calibration pulls the other way) have sat near H₀ ≈ 69.9 on Planck 2018 + ACT DR6 + SPT-3G + BAO + Pantheon+SH0ES. **Do not quote H₀ ≈ 69.9, any “outperform ΛCDM” claim, or a best-fit comparison as a result.** Stage A MCMC is booked. On SH0ES, nested sampling got close enough to compare the twins, not far enough to book a full sampler. No nested ΔlnZ. The value is SH0ES-conditional and not a nested verdict.
...
vs ΛCDM at full freedom. On SH0ES, nested sampling got close enough to compare the twins, not far enough to book a full sampler. No nested ΔlnZ. Until a nested twin finishes: Laplace-from-MCMC. Caveat: transition epoch frozen at a **profiled** value 0.053 dex from the
...
| 4 | Fit status / multi-basin stopped run not quotable; old-BAO pair later booked but does **not** yield a strong evidence win | **honest constraint** | status section; booking receipt | **OPEN-NESTED:** booked old-BAO sample-cov Laplace only ≈ +0.21; DESI-DR2 Stage A booked; SH0ES nested: close enough to compare, not booked |
