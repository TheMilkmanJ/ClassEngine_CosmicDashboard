...
**Zero-parameter evidence test.** Amplitude, tilt, coupling, and transition epoch frozen in advance vs ΛCDM at full freedom. Nested sampling started then stood down on the laptop it first ran on; the current AWS nested state is a **dyad evidence leg on the replacement `c7i.24xlarge` Spot box at `96` ranks that is still up but watcher-marked STALLED, with the repaired ΛCDM twin worker waiting behind it and no verdict yet**. Until that pair finishes: Laplace-from-MCMC. Caveat: transition epoch frozen at a **profiled** value 0.053 dex from the model’s onset identity (~28% in dark-fluid mass). **The graded configuration is near the model, not the stated one.**
...
| 4 | Fit status / multi-basin stopped run not quotable; H₀ ≈ 69.9 and “outperform” demoted; live R−1 lcdm **0.049324**@N=26294 t=2026-08-05T11:52:10 (`converged:true`; control leg ready) / dyad **0.085619**@N=27525 t=2026-08-05T22:03:22 (**1.71×**, `converged:false`) | **honest constraint** | status section; progress | **NOT bookable**; no peek H₀ as result; joint waits `dyad_mnu_bbnfix` book via `book_bbnfix_when_ready.py` |
