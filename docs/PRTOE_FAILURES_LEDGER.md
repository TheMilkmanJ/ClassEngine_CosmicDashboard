...
| **THE G-CLOSURE ITSELF (the induced-G match: "Newton's constant from the roster at 0.8%")** | **DEAD — TERMINATION-B (entry 109).** The pre-registered anisotropic-heat-kernel computation returns str ≈ 3 (epoch-dependent — fatal on its own) or 10 (most generous reading) against the 12π ≈ 37.7 demand. The celebrated 38 rested on TWO bookkeeping errors, both found by this project's own audits: the twin double-count (adjudicated away, entry 99) and a large negative ξ that the direct tensor computation shows does not exist (entry 108). With both corrected the match evaporates. **The QG framework survives; the closure does not. Every public claim resting on it requires correction.** | entries 78–109, killed 2026-07-13 by its own pre-registration |
