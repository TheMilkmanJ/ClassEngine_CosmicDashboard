...
The fit gives **A = R/M = 1.41420** (reconstruction exact). The Z₃ geometry gives Q = 1/3 + A²/6
...
model's arithmetic circles Koide's rationals without landing them. *(The retired f_amp
decomposition, ε = c × f_amp × Ψ₀/M_red, is archive-only and is not a check on anything —
[PRTOE_INDEX.md](PRTOE_INDEX.md).)*
