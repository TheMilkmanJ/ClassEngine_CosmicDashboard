...
The cosmic microwave background carries three fields on the sky:
...
Before recombination, photons and baryons are one fluid. It falls into the
...
~69.9. TT is reproduced at ΛCDM quality; the m_e tweak is the live signal.

## 2. TE — temperature × E-mode

E-mode polarization is made at last scattering: Thomson scattering off a
...
Same fluid, same wells, same recombination (m_e-shifted). Fit in the MCMC
alongside TT.

## 3. EE — E-mode × E-mode

The auto-power of the parity-even (gradient) polarization. **Scalar (density)
...
*(This spectrum is also the raw material any birefringence would act on —
see §6.)*

## 4. BB — B-mode × B-mode

The parity-odd (curl) polarization. The key fact: **scalar/density
...
- *Lensing BB:* follows automatically from the model's matter power spectrum (dCDF
 clustering → lensing potential) — inherited, and fit.
...
Correlating a parity-even field (T or E) with the parity-odd field (B).
...
TB and EB are **not independent signals** — they are the
...
The medium is **electrically neutral in every state and every regime**, so it
...
|---|---|---|---|
| **TT** | even | acoustic oscillations in dCDF wells + SW/ISW/Silk | reproduced in the fit (m_e shift = live signal) |
...
| **TB** | odd | E rotated into B by β | predicted zero (β = 0) |
| **EB** | odd | E rotated into B by β | predicted zero (β = 0) |

Everything parity-**even** — the entire bulk of the measured CMB —
...
The CMB, mechanism by mechanism, with the model's verdict on each — and a
