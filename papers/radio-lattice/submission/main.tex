...
ones. We treat $\varepsilon$ as a free amplitude to be fitted, not predicted, so the ratio
structure can be tested without committing to a mechanism for the shift. Read through the pattern,
the tightest existing $21\,$cm comparison gives $|\varepsilon| < 1.4 \times 10^{-5}$ at $2\sigma$
over $1.17 < z < 1.56$, limited by systematics rather than by precision; molecular lines bound it
more tightly still. The pattern is a fingerprint of the shift, not a better measurement of its size.
A single band deviating from its assigned weight falsifies the pattern.
\end{abstract}
...
sensitivity, which rows can be entered today, and where the resulting bound sits beside the best
existing one.
% ---------------------------------------------------------------------------
...
amplitude does not diminish it. The methanol analyses already illustrate the same point that
limits the $21\,$cm row: a tighter statistical limit from one line pair was set aside in favour of
the weaker figure from three transitions whose profiles agree, on the grounds that only those
demonstrably sample the same gas. Both of the best available constraints on $\varepsilon$ are
