...
\begin{abstract}
Two-color QCD has a substantial finite-temperature literature at $N_f=2$, yet no published
determination of the transition temperature in string-tension units, $T_c/\sqrt{\sigma}$, exists
for $N_f=3$ light fundamental Dirac flavours. We state the computation cleanly: separate chiral and
deconfinement crossovers, a chiral extrapolation, a continuum statement at least two temporal
extents, and an explicit confrontation of the staggered-rooting versus Wilson trade-off in a
pseudo-real theory. The gap is scientifically interesting independent of any particular dark-sector
model: flavour dependence of $T_c/\sqrt{\sigma}$ is well mapped in SU(3), while SU(2)'s enlarged
chiral pattern and different Goldstone count leave the saturation of that flavour dependence open.
We record, for transparency only, a publicly pre-registered numerical stake on the chiral
transition and the precision required to score it; the stake is not the result of this note. The
result of the note is that the calculation is conventional, well posed, and missing.
\end{abstract}
...
What is missing is $N_f=3$ light fundamentals. $N_f=4$ exists only in qualitative form from older
work. That absence matters for three reasons that do not require a dark-sector story.
...
precision discussed next.

\section{A pre-registered stake (transparency only)}

An independent phenomenological program has publicly pre-registered a numerical stake on the chiral
transition, placed before any dedicated $N_f=3$ computation existed:
$T_c/\sqrt{\sigma}=0.34657$ ($=\tfrac12\ln 2$), with pre-committed kill conditions. The stake is
recorded here for transparency, not as a result of this note.

The precision required to score the stake is more important than the central value. The stake
distinguishes two hypotheses $0.44\%$ apart; a determination with relative uncertainty $\sigma>0.44\%$
cannot score either way and is registered in advance as inconclusive. Discrimination needs
$\sigma\lesssim 0.22\%$. At ordinary published precision ($1$--$3\%$), the expected outcome is
therefore a null score on the stake itself.

Two coarser clauses remain live at ordinary precision: a determination outside $[0.330,0.370]$
falsifies that program's dark-energy sector, and the best current inference from measured
neighbours is $0.39\pm 0.05$, centred \emph{above} the stake. Whether those clauses matter is the
program's problem. The field's problem is the missing $N_f=3$ number.

\section{Conclusion}

No published $T_c/\sqrt{\sigma}$ exists for SU(2) with $N_f=3$ light fundamental flavours. The
calculation is conventional, well posed, and discretization-sensitive. Filling the gap would answer
a clean question about flavour dependence in a pseudo-real theory, independent of any particular
beyond-Standard-Model motivation. We have stated the requirements, the literature anchors at
$N_f=2$, and the precision a pre-registered stake would need. The computation itself remains to be
done.
