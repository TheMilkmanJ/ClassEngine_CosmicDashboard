...
afterwards. That program's own registry states that discriminating its stake from a nearby
observation-inverted null (clauses 2/3) is \emph{not} executable at present cosmological
precision---the limit is the sky's $\rho_\Lambda$ error, not lattice effort---so a demand of
$\sigma\lesssim 0.22\%$ lattice precision for that fork is \emph{not} what the registration
asks of the field. What remains fully executable without that null is a coarse window kill
(clause~4: determination outside a fixed band). Coarser falsification windows stated by that
program are its problem, not the field's. The field's problem is the missing $N_f=3$ number.
