...
| DESI-DR2 FD Hessian | `i-096d08d2dc9d8f42c` 48 | **LAUNCHING / running** → `hessian_laplace_desi.json` on box; pull when done |

Still **not nested**. Stop DESI box when DESI JSON lands to save cost.
...
| DESI Hessian | `i-096d08d2dc9d8f42c` 48 | FD Laplace on booked DESI chains |

Quota 300; ~240 in use. TRGB PC needs free ≥96 (not available while 2×96 gold PC hold).

### MCMC dual-gates (done)

| Pair | Receipt |
|---|---|
| Old-BAO bbnfix | `bbnfix_booking_20260808_005626` |
| DESI-DR2 bbnfix | `desidr2_bbnfix_booking_20260810_053127` |

Stage B table publish still wants red audit if you want automated shelf write.
