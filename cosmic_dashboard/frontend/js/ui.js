...
async function fetchSysInfo() {
    try {
        const response = await CosmicAPI.getSysInfo();
...
async function fetchMultimodalComparison() {
    ...
    try {
        const response = await CosmicAPI.getMultimodalComparison();
        if (!response.ok) {
...
async function checkStatus() {
    ...
    try {
        const response = await CosmicAPI.getStatus(controller);
...
async function refreshCompare() {
    ...
    try {
        const response = await CosmicAPI.compareModels();
        ...
        data.models.forEach(m => {
            const chi2Str = (m.chi2 !== null && typeof m.chi2 === "number") ? m.chi2.toFixed(2) : "-";
            const logzStr = m.logz !== null ? `${m.logz.toFixed(2)} +/- ${m.logz_err.toFixed(2)}` : "-";
            const h0Str = (m.h0_tension !== null && typeof m.h0_tension === "number") ? `${m.h0_tension.toFixed(2)}σ (${m.h0_val.toFixed(2)})` : ((m.h0_val !== null && typeof m.h0_val === "number") ? m.h0_val.toFixed(2) : "-");
            const s8Str = (m.s8_tension !== null && typeof m.s8_tension === "number") ? `${m.s8_tension.toFixed(2)}σ (${m.s8_val.toFixed(3)})` : ((m.s8_val !== null && typeof m.s8_val === "number") ? m.s8_val.toFixed(3) : "-");
...
async function runStabilityScanner() {
    ...
    try {
        const response = await CosmicAPI.runStabilityScan(activeConfig);
...
async function runSensitivityAnalyzer() {
    ...
    try {
        const response = await CosmicAPI.runSensitivityAnalysis(activeConfig);
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.status === "success") {
                ...
                obs.forEach(o => {
                    html += `<th style="padding: 6px;">${escHtml(String(o))}</th>`;
                });
                html += `</tr>`;
                
                params.forEach(p => {
                    html += `<tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">`;
                    html += `<td style="padding: 6px; text-align: left; font-weight: bold; color: #ff9ff3;">${escHtml(String(p))}</td>`;
                    obs.forEach(o => {
                        ...
                        html += `<td style="padding: 6px; background: ${bg}; color: ${color};" title="&part;ln(${escHtml(String(o))})/&part;ln(${escHtml(String(p))}) = ${val.toFixed(5)}">${val.toFixed(4)}</td>`;
                    });
                    html += `</tr>`;
                });
                html += `</table>`;
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.status === "success") {
                ...
                params.forEach(p => {
                    html += `<th style="padding: 4px; font-size: 0.7rem;" title="${escHtml(String(p))}">${escHtml(String(p).substring(0, 8))}</th>`;
                });
                html += `</tr>`;
                
                params.forEach((p1, idx1) => {
                    html += `<tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">`;
                    html += `<td style="padding: 4px; text-align: left; font-weight: bold; color: #ff9ff3; font-size: 0.7rem;">${escHtml(String(p1).substring(0, 8))}</td>`;
                    ...
                    params.forEach((p2, idx2) => {
                        ...
                        html += `<td style="padding: 4px; background: ${bg}; color: #fff;" title="Covariance(${escHtml(String(p1))}, ${escHtml(String(p2))}) = ${val.toExponential(3)}\nCorrelation = ${corr.toFixed(3)}">${corr.toFixed(2)}</td>`;
                    });
                    html += `</tr>`;
                });
                html += `</table>`;
...
function playIntergalacticSynth() {
    appendLog("<span style='color: #00d2ff;'>🌌 [AUDIO MUTED BY ADMIN] Celebration complete! 🌌</span>", { html: true });
